...
import negocios.CriptografiaMD5;
...
public class Formatacao {
     ...
     public static Date ConvercaoStringData(String data){
      ...
      Date dataFormatada = null;
        try {
            ...
            Logger.getLogger(Sigap.class.getName()).log(Level.SEVERE, null, ex);
        }
  ...
  public static String SubtarirDatas(Date data, int dias, String formato) throws Exception{
    ...
    public static boolean validarSenha(String login, char[] senha) {
        Usuarios usuarioLogado = new Usuarios();
        String validaSenha = null;
        if (senha.length > 0) {
            UsuariosController usuariosController = new UsuariosController();
            validaSenha = Formatacao.criptografarSenha(senha);
            usuarioLogado = usuariosController.consultarUsuario(login, validaSenha);
            if (usuarioLogado == null) {
                JOptionPane.showMessageDialog(null, "Usuario e/ou Senha incorreta.");
            }
        } else {
            if (!usuarioLogado.getSenha().equalsIgnoreCase(validaSenha)) {
